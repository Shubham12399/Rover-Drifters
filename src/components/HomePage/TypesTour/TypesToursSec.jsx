...
const TypesToursSec = () => {
    const {typesTours,isLoading} = useFetch({queryKey:"typesTours",url:typeApi.GET_ALL_TYPE,varName:"typesTours"});
  ...
  return (
    <div>
       {
        typesTours?.map(e=>(
            <TypeTour _id={e._id} title={e.name} paragraph={e.paragraph} key={e._id} tours={e.tours}></TypeTour>
        ))
       }
    </div>
  )
